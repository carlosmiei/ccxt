...
export default class binance extends Exchange {
    ...
    async fetchMarkets(params = {}) {
        ...
        for (let i = 0; i < rawTopicsLength; i++) {
            ...
            const eventMarkets = this.safeList(parsedEvent, 'markets', []);
            const eventMarketsLength = eventMarkets.length;
            for (let mi = 0; mi < eventMarketsLength; mi++) {
    ...
    async fetchRawTopics(maxTopics, rest = {}) {
        ...
        while (true) {
            ...
            //     }
            //
            const pageTopics = this.safeList(response, 'marketTopics', []);
     ...
     */
    async fetchRawTopicDetail(topicId, params = {}) {
        const request = {
     ...
     */
    async completeRawTopics(rawTopics) {
        const result = [];
        const rawTopicsLength = rawTopics.length;
        for (let i = 0; i < rawTopicsLength; i++) {
            const rawTopic = rawTopics[i];
            const rawMarkets = this.safeList(rawTopic, 'markets', []);
            const rawMarketsLength = rawMarkets.length;
            let hasOutcomes = false;
            if (rawMarketsLength > 0) {
                const firstMarket = this.safeDict(rawMarkets, 0, {});
                const firstOutcomes = this.safeList(firstMarket, 'outcomes', []);
    ...
    async fetchEvents(params = {}) {
        ...
        for (let i = 0; i < rawTopicsLength; i++) {
            ...
            const parsedMarkets = this.safeList(parsedEvent, 'markets', []);
    ...
    async fetchEvent(id, params = {}) {
        ...
        const event = this.parseEvent(detail);
     ...
     */
    parseTopicMarket(rawMarket, rawTopic) {
        //
        //     {
        //         "marketId": 5567895,
        //         "externalId": "ext_001",
        ...
        const rawOutcomes = this.safeList(rawMarket, 'outcomes', []);
        const outcomes = [];
     ...
     */
    sign(path, api = 'sapi', method = 'GET', params = {}, headers = undefined, body = undefined) {
        const apiGroup = typeof api === 'string' ? api : api[0];
        const baseUrls = this.urls['api'];
        const baseUrl = this.safeString(baseUrls, apiGroup, baseUrls['sapi']);
