...
export default class Kalshi extends Exchange {
    ...
    parseBinaryMarketToOutcomes (raw: Dict): Market[] {
        ...
        for (const outcome of outcomes) {
            const symbol = ticker + '/' + outcome.label + ':USD';
            result.push ({
                'id':             outcome.id,
                'symbol':         symbol,
                'base':           outcome.label,
    ...
    async fetchEvents (params: Dict = {}): Promise<any[]> {
        ...
        const rest  = this.omit (params, ['id', 'status']);

        let rawEvents: any[] = [];

        if (id) {
            const response = await this.kalshiPublicGetEventsEventTicker (this.extend ({ 'event_ticker': id }, rest));
            rawEvents = [this.safeValue (response, 'event', response)];
        } else {
            ...
            const response = await this.kalshiPublicGetEvents (this.extend ({
                'status': status,
                'limit':  limit,
            }, rest));
